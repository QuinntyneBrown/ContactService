...
namespace ContactService.API.Features.Contacts
{
    public class UpdateContactCommand
    {
        ...
        public class Handler : IRequestHandler<Request, Response>
        {
            ...
            {
                var contact = _eventStore.Query<Contact>(request.Contact.ContactId);

                contact.ChangeName(request.Contact.Firstname);

                _eventStore.Save(contact);

                return Task.FromResult(new Response() { ContactId = request.Contact.ContactId }); 
            }
        }
    }
}
